        ...
        <div class="b-listSearchForm">
            <div class="b-listSearchForm__inner">
                    ...
                    <div class="b-form-group">
                        <div class="b-form-group__item">
                            ...
                            <div class="inline-group">
                                <select name="searchAccountType" class="default-input w100">
                                    <option class="trn" data-trn-key="TR_ACCOUNT_MNG_AZURE_ACCOUNT_TYPE" value=""><%--계정 구분--%></option>
                                    <option value="E">Exchange</option>
                                    <option value="O">Onedrive</option>
                                </select>
                                <select name="searchType" class="default-input w100">
                                    <option class="trn" data-trn-key="TR_COMMON_SEARCH_ALL" value=""><%--전체--%></option>
                                    <option class="trn" data-trn-key="TR_COMMON_ACCOUNT" value="account"><%--계정--%></option>
                                </select>
                ...
                <form id="pageForm">
                    ...
                    <input type="hidden" name="searchAccountType" value="${paging.searchAccountType}">
                </form>
                ...
                <div class="b-tableWrap">
                    <table class="b-listTable plus">
                        ...
                        <tr>
                            ...
                            <th class="trn" data-trn-key="TR_ACCOUNT_MNG_CUSTOMER_COMPANY_IN_USE"><%--사용중인 고객사--%></th>
                        ...
                        <c:if test="${totalCount == 0}">
                            <tr>
                                <td colspan="6" class="nodata trn" data-trn-key="TR_COMMON_NOTDATALIST"><%--데이터가 존재하지 않습니다.--%></td>
                            </tr>
                        </c:if>
                        <c:forEach var="azureAccount" items="${azureAccountList}" varStatus="status">
                            <tr class="pointer" onclick="moveView(${azureAccount.idx})">
                                <td class="a_center">${(totalCount-status.index) - ((paging.pageNo-1) * paging.pageSize)}</td>
                                <td class="a_center">
                                    ${azureAccount.customerName}
                                    <c:if test="${azureAccount.customerName == null}">
                                        -
                                    </c:if>
                                </td>
                                <td class="a_center">${azureAccount.account}</td>
                                <td class="a_center">${azureAccount.accountType == "E" ? "Exchange" : "Onedrive"}</td>
                                <td class="a_center">
                                    <fmt:formatNumber value="${azureAccount.accountType == 'E' ? azureAccount.exchangeStorage : azureAccount.onedriveStorage}" pattern="#,###"/> GB
                                </td>
                                </td>
                                <td class="a_center">${azureAccount.formatedRegDate}</td>
                                <td class="a_center">${azureAccount.regId}</td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
