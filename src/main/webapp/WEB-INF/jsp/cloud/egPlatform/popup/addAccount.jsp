...
<script src="/resources/js/cloud/egPlatform/popup/addAccount.js" type="text/babel"></script>
            ...
            <%--<hr class="subLine">--%>
                <input type="hidden" name="idx" value="${customer.idx}">
                                    ...
                                    <tr>
                                        <th>
                                            <select id="exchangeAccount" class="w200">
                                                ...
                                                <c:forEach var="exchangeAccount" items="${exchangeAccountList}" varStatus="status">
                                                    <c:if test="${!fn:contains(selectedIdx, exchangeAccount.idx)}">
                                                        <option value="${exchangeAccount.idx}">${exchangeAccount.account}</option>
                                                    </c:if>
                                                </c:forEach>
                                            </select>
                                            <input type="button" class="trn" data-trn-value="TR_COMMON_ADD" value="" onclick="addExchangeAccountBtn()"><%--추가--%>
                                        </th>
                                    </tr>
                                </thead>
                                        ...
                                        <c:if test="${fn:contains(selectedIdx, exchangeAccount.idx)}">
                                            <tr>
                                                <td style="display: flex; justify-content: space-between; align-items: center; height: auto" class="clear">
                                                    ...
                                                    <span class="text dis_ib w200 exchangeId">${exchangeAccount.account} <span class="fwB">
                                                    <c:if test="${customer.secuEcloudGubun == 'N'}">
                                                        [${usedStorage}GB / ${exchangeStorage}GB]
                                                    </c:if>
                                                </span></span>
                                                    <span><span class="trn" data-trn-key="TR_COMMON_USEYN_TRUE"><%--사용--%></span> : <input type="radio" value="true"${exchangeAccount.exchangeSaveYn ? ' class="check"' : ''} name="exchangeSaveYn" onchange="radioChange('exchange', this)" ${exchangeAccount.idx == pointIdx ? 'checked' : ''}> </span>
                    ...
                    <p class="clear a_center mt10" style="overflow:hidden;">
                        <input type="button" class="btn-large btn-add trn" data-trn-value="TR_COMMON_REGISTRATION" onclick="addAccount();" value=""><%--등록--%>
