...
<script src="/resources/js/customerMng/partner/manager/list.js" type="text/babel"></script>
<!--header, leftMenu 제외한 컨텐츠 영역 <section class="l-content-area">-->
<section class="l-content-area">

    <!--메인타이틀 영역 <h1 class="main-title">-->
    <h1 class="main-title">파트너사 관리 > 담당자 관리</h1>
            ...
            <div class="b-listSearchForm__inner">
                <form id="searchForm">
                    ...
                    <div class="b-form-group">
                        <div class="b-form-group__item">
                            <div class="inline-group">
                                <select name="searchType" class="default-input w120">
                                    ...
                                    <option value="id" ${param.searchType eq 'id' ? 'selected' : ''}>아이디</option>
                        ...
                        <input type="button" class="btn-baseRound fr" value="파트너사 목록" onclick="moveList();">
                    </div>
                </form>
                <form id="pageForm">
                ...
                <div class="b-tableWrap">
                    <table class="b-listTable">
                        ...
                        <c:forEach var="manager" items="${managerList}" varStatus="status">
                            <tr class="pointer" onclick="savePopup(${manager.idx})">
                                <td class="a_center">${(totalCount-status.index) - ((paging.pageNo-1) * paging.pageSize)}</td>
                                <td class="a_center">${manager.name}</td>
                                <td class="a_center">${manager.id}</td>
                                <td class="a_center">${manager.phone}</td>
                                <td class="a_center">${manager.email}</td>
                                <td class="a_center">
                                    <fmt:formatDate value="${manager.regDate}" pattern="yyyy-MM-dd HH:mm"/>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
