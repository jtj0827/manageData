...
<script type="text/babel" src="/resources/js/data/urlBlackList.js"></script>

<section class="l-content-area">

    <!--메인타이틀 영역 <h1 class="main-title">-->
    <h1 class="main-title">URL 블랙리스트</h1>
                        ...
                        <div class="b-form-group__item">
                            ...
                            <div class="inline-group">
                                <select name="searchType" class="default-input w100">
                                    ...
                                    <option value="url" ${param.searchType eq 'url' ? 'selected' : ''}>URL</option>
                                    <option value="explanation" ${param.searchType eq 'explanation' ? 'selected' : ''}>설명</option>
                                </select>
                                <input type="text" class="default-input w120" name="searchText" value="${param.searchText}" maxlength="100">
                            </div>
                        </div>
        ...
        <div class="l-content">
            <div class="b-list">
                <div class="b-list-top">
                    ...
                    <input type="button" class="btn-baseRound trn" onclick="urlBlackListPopup()" value="등록">
                ...
                <div class="b-tableWrap">
                    <table class="b-listTable">
                        ...
                        <thead>
                        <tr>
                            <th><input type="checkbox" name="urlBlackListCheckGubun" class="urlBlackListCheckGubun" onchange="allCheckedBox()"></th>
                            <th>URL</th>
                            <th>설명</th>
                            <th>작성자</th>
                            <th>등록일</th>
                        ...
                        <c:if test="${totalCount ne 0}">
                            <c:forEach items="${urlBlackList}" var="data" varStatus="status">
                                <tr class="pointer">
                                    <td class="a-center">
                                        <input type="checkbox" name="urlBlackListCheckGubun" class="urlBlackListCheckGubun" value="${data.idx}">
                                    </td>
                                    <td class="a-center" onclick="urlBlackListPopup('${data.idx}')">${data.url}</td>
                                    <td class="a-center" onclick="urlBlackListPopup('${data.idx}')">${data.explanation}</td>
                                    <td class="a-center" onclick="urlBlackListPopup('${data.idx}')">${data.regId}</td>
                                    <td class="a-center" onclick="urlBlackListPopup('${data.idx}')"><fmt:formatDate value="${data.regDate}" pattern="yyyy-MM-dd HH:mm"/></td>
                                </tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
